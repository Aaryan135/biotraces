/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VaccinationWhereInput } from "./VaccinationWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { VaccinationOrderByInput } from "./VaccinationOrderByInput";

@ArgsType()
class VaccinationFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => VaccinationWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => VaccinationWhereInput, { nullable: true })
  @Type(() => VaccinationWhereInput)
  where?: VaccinationWhereInput;

  @ApiProperty({
    required: false,
    type: [VaccinationOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [VaccinationOrderByInput], { nullable: true })
  @Type(() => VaccinationOrderByInput)
  orderBy?: Array<VaccinationOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { VaccinationFindManyArgs as VaccinationFindManyArgs };
